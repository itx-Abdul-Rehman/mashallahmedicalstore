...
export default function AdminManageMedicines() {
    ...
    const handleDelete = async () => {
        try {
            const id = deleteMedicine.id;
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/delete/medicine?id=${id}`, {
                method: 'DELETE',
                headers: {
    ...
    const alertDialog = (index) => {
        if (searchData.length > 0) {
            setDeleteMedicine(searchData[index]);
        } else {
            setDeleteMedicine(medicinesData[index]);
        }
        setOpen(true);
    }
